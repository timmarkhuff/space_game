...
game_controller = pd.DataFrame()
# get the game controller from Google Sheets if possible, otherwise get it from Git Hub
try:
    # AUTENTICATE GOOGLE
    # define the scope
    ...
    json = os.path.join('Assets', 'api-project-688089548546-3fb0fab48f0a.json')
    ...
    sheet = client.open("Space Game Controller")
    ...
    game_controller = pd.DataFrame.from_dict(data)
    print('getting data from google')
    print(game_controller)

except:
    print('getting data from git hub')
    game_controller = pd.read_csv("https://raw.githubusercontent.com/timmarkhuff/data/main/game_controller.csv")
    print(game_controller)
...
class ship_1:
    ...
    def ai_move_to_target(self):
        ...
        """
        # commenting out the threading code. I don't think this needs to be a thread
        # def thread():
        #     global run, round_is_ongoing
        #     buffer = self.target_rectangle.width//2
            
        #     clock = pygame.time.Clock()
        #     while run and round_is_ongoing:
        #         clock.tick(FPS)
...
def write_to_leaderboard():
    ...
    # get the instance of the Spreadsheet
    sheet = client.open("Space Game Leader Board")

    # get the instance of the second sheet
    leaderboard = sheet.get_worksheet(0)

    # write to the leaderboard
    now = str(datetime.now())
    username = os.getlogin()
    fuel = str(player_ship.fuel) # for some reason, Google won't write this value to the sheet unless it is a string
    leaderboard.insert_rows([[now, username, ROUND_COUNT, fuel, version]], row=2)
...
def buy_upgrades():
    ...
    # replenish some stats so that the user can see their full stats while buying upgraedes
